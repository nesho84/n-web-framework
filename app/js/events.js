...
let calendar = null;
let eventsUrl = window.location.href;
...
//------------------------------------------------------------
{
    const requestUrl = `/deleteJson/${eventId}`;
    const requestBody = {
...
//------------------------------------------------------------
{
    Swal.fire({
       ...
       <input id="swalEvtURL_edit" class="form-control mb-3" placeholder="Enter URL" value="${event.url}">`,
        ...
        if (result.value) {
            const requestUrl = `/updateJson/${event.id}`;
            const requestBody = {
